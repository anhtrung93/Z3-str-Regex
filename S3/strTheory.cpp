...
Z3_ast my_mk_str_value(Z3_theory t, char const * str) {
	...
	if (constStr_astNode_map.find("") == constStr_astNode_map.end()) {
		...
		Z3_ast emptyStrNode = Z3_theory_mk_value(ctx, t, empty_str_sym,
				td->String);
...
T_myZ3Type getNodeType(Z3_theory t, Z3_ast n) {
	...
	case Z3_APP_AST: {
		...
		if (Z3_theory_is_value(t, n)) {
			...
			if (sk == Z3_BOOL_SORT) {
				if (d == td->Contains || d == td->StartsWith
						|| d == td->EndsWith || d == td->In) {
					return my_Z3_Func;
				} else {
			...
			if (sk == Z3_INT_SORT) {
				if (d == td->Length || d == td->Indexof || d == td->Indexof_I
						|| d == td->Search) {
					return my_Z3_Func;
				}
...
T_myZ3Type getNodeTypeCustom(Z3_theory t, Z3_ast n) {
	...
	case Z3_APP_AST: {
		if (Z3_theory_is_value(t, n)) {
			Z3_func_decl d = Z3_get_app_decl(ctx, Z3_to_app(ctx, n));
			if (d == td->Contains || d == td->Indexof || d == td->Indexof_I
					|| d == td->Replace || d == td->In || d == td->Star)
				return my_Z3_Unknown;
			else if (d == td->Star_I)
				return my_Z3_Star;
			else if (d == td->Concat)
...
Z3_ast mk_concat(Z3_theory t, Z3_ast n1, Z3_ast n2) {
	...
	if (n1 == NULL || n2 == NULL) {
		fprintf(stdout,
				"> Error: the strings to be concat cannot be NULL (@ %d).\n",
				__LINE__);
		exit(0);
	} else {
...
Z3_ast mk_union(Z3_theory t, Z3_ast n1, Z3_ast n2) {
	...
	if (n1 == NULL || n2 == NULL) {
		fprintf(stdout,
				"> Error: the parameters of UNION cannot be NULL (@ %d).\n",
				__LINE__);
		exit(0);
	} else {
		n1 = get_eqc_value(t, n1);
...
Z3_ast mk_star(Z3_theory t, Z3_ast n1) {
	...
	if (n1 == NULL || n2 == NULL) {
		fprintf(stdout,
				"> Error: the parameters of STAR cannot be NULL (@ %d).\n",
				__LINE__);
		exit(0);
	} else {
		n1 = get_eqc_value(t, n1);
...
Z3_ast mk_idxof(Z3_theory t, Z3_ast n1, Z3_ast n2) {
	...
	if (n1 == NULL || n2 == NULL) {
		fprintf(stdout,
				"> Error: the parameters of INDEX_OF cannot be NULL (@ %d).\n",
				__LINE__);
		exit(0);
	} else {
...
Z3_ast mk_search(Z3_theory t, Z3_ast n1, Z3_ast n2) {
	...
	if (n1 == NULL || n2 == NULL) {
		fprintf(stdout,
				"> Error: the parameters of SEARCH cannot be NULL (@ %d).\n",
				__LINE__);
		exit(0);
	} else {
...
Z3_ast mk_replaceAll(Z3_theory t, Z3_ast const args[]) {
	if (!tempPatch)
		tempPatch = true;
	Z3_context ctx = Z3_theory_get_context(t);
	PATheoryData * td = (PATheoryData*) Z3_theory_get_ext_data(t);
	Z3_ast n1 = get_eqc_value(t, args[0]);
	Z3_ast n2 = get_eqc_value(t, args[1]);
	Z3_ast n3 = get_eqc_value(t, args[2]);
	// TODO: Simplified version and Length
...
Z3_ast mk_subStr(Z3_theory t, Z3_ast const args[]) {
	...
	if (subStr_astNode_map.find(subStrArgs) == subStr_astNode_map.end()) {
		...
		//#ifdef DEBUGLOG
		//        __debugPrint(logFile, "[SubString made] ");
...
void solve_replaceall_eq_sth(Z3_theory t, Z3_ast replaceAllAst, Z3_ast res) {
	...
	if (isConstStr(t, arg2) && isConstStr(t, arg3)) {
		...
		and_items2[3] = Z3_mk_eq(ctx, remres, mk_replaceAll(t, para));
		...
		Z3_ast para[3] = { rem, arg2, arg3 };
		and_items2[5] = Z3_mk_eq(ctx, remres, mk_replaceAll(t, para));
...
Z3_bool cb_reduce_app(Z3_theory t, Z3_func_decl d, unsigned n,
	...
	if (d == td->ReplaceAllExt) {
		*result = mk_replaceAll(t, convertedArgs);
#ifdef DEBUGLOG
		__debugPrint(logFile, "\n===================\n");
